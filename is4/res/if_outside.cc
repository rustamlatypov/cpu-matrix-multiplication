        ...
        for (int i0 = 0; i0 < nx; i0++) {
            ...
            for (int j1 = j0+1; j1 < ny+1; j1++) {
                ...
                for (int i1 = i0+1; i1 < nx+1; i1++) {
                    ...
                    double h_value = vx[0] + vx[1] + vx[2] + vy[0] + vy[1] + vy[2];

                    candidates.push_back({ h_value,j0,i0,j1,i1 });
                }

                int thread = omp_get_thread_num();
                Req current = req_v[thread];

                for (long unsigned int m = 0; m < candidates.size(); m++) {
                    if (candidates[m].h_value > current.h_value) {
                        current = candidates[m];
                    }
                }
