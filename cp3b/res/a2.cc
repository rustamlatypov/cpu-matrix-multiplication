    ...
    */
    ...
    for (int j = 0; j < nyb; j++) {
        ...
        for (int i = j; i < nyb; i++) {
            ...
            float8_t block[A*A*P] = {float8_0};

            asm("#foo");
            for (int k = 0; k < nx; k++) {

                //__builtin_prefetch(&aux[(j*A)*nx + k + 10]);
                float8_t a0 = aux[(j*A)*nx + k];
                float8_t a1 = aux[(j*A+1)*nx + k];

                float8_t b0 = aux[(i*A)*nx + k];
                float8_t b1 = aux[(i*A+1)*nx + k];
